        ...
        <article>
            ...
            <ul class="post-meta mb-2 mt-4">
                <li>
                    ...
                    </svg> <span><?= convert_time_dmy($post->updated_at) ?></span>
                </li>
            </ul>
            <h1 class="my-3"><?= $post->title ?></h1>
            <?php if (count(get_tags_single_post($post->id))): ?>
                <ul class="post-meta mb-4">
                    <?php foreach (get_tags_single_post($post->id) as $tag_single_post): ?>

                        <li>
                            <a href="/tag/<?= $tag_single_post ?>"><?= $tag_single_post ?></a>
                        </li>
                    <?php endforeach ?>
                </ul>
            <?php endif ?>
                ...
                <div class="mt-3" id="related-posts">
                    ...
                    <div class="widget-body">
                        <div class="widget-list">
                            <?php foreach (get_related_post_by_post_id($post->id) as $related_post): ?>
                                <a class="media align-items-center" href="<?php route_to('read-post', $related_post->slug) ?>">
                                    <img loading="lazy" decoding="async"
                                        src="/images/posts/thumb_<?= $related_post->featured_image ?>" alt="Post Thumbnail"
                                        class="w-100">
                                    <div class="media-body ml-3">
                                        <h3 style="margin-top:-5px"><?= $related_post->title ?></h3>
                                        <p class="mb-0 small"></p>
                                        <p><?= wrapper_limit_text($related_post->content) ?></p>
                                    </div>
                                </a>
                            <?php endforeach ?>
                        </div>
            ...
            <script>
                ...
                *  LEARN WHY DEFINING THESE VARIABLES IS IMPORTANT: https://disqus.com/admin/universalcode/#configuration-variables    */
                ...
                (function () { // DON'T EDIT BELOW THIS LINE
                    ...
                    s.src = 'https://ci4blog-1.disqus.com/embed.js';
...
<link rel="stylesheet" href="/extra-assets/social-media-pluggin/jquery.floating-social-share.min.css">
<?= $this->endSection() ?>

<?= $this->section('scripts') ?>
<script src="/extra-assets/social-media-pluggin/jquery.floating-social-share.min.js"></script>
<script>
    $("body").floatingSocialShare(
        {
            buttons: ["facebook", "twitter", "odnoklassniki", "tumblr", "viber", "telegram", "linkedin", "pinterest"],
            url: "<?= current_url() ?>"
        }
    );
